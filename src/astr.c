...
int
astr_find (const astr as, const astr src)
{
  return astr_find_cstr (as, src->text);
}

int
astr_find_cstr (const astr as, const char *s)
{
  char *sp;
...
astr
astr_replace (astr as, ptrdiff_t pos, size_t size, const astr src)
{
  assert (src != NULL);
...
astr
astr_replace_char (astr as, ptrdiff_t pos, size_t size, int c)
{
  return astr_replace_x (as, pos, size, (const char *) &c, (size_t) 1);
}

astr
astr_insert (astr as, ptrdiff_t pos, const astr src)
{
  assert (src != NULL);
...
astr
astr_insert_cstr (astr as, ptrdiff_t pos, const char *s)
{
  assert (s != NULL);
