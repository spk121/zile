...
flush (rbacc rba)
{
  ...
  if (rba->used > 0)
    rba->head =
      rblist_concat (rba->head, rblist_from_array (rba->tail, rba->used));
  rba->used = 0;
}
...
rbacc
rbacc_add_char (rbacc rba, int c)
{
...
rbacc
rbacc_add_array (rbacc rba, const char *cs, size_t length)
{
...
rbacc
rbacc_add_string (rbacc rba, const char *s)
{
  return rbacc_add_array (rba, s, strlen (s));
}

rbacc
rbacc_add_number (rbacc rba, size_t x, unsigned base)
{
...
rbacc
rbacc_add_file (rbacc rba, FILE * fp)
{
  int c;
 ...
 */
static void
test (rbacc rba, const char *s, size_t length)
{
  assert (rbacc_length (rba) == length);
  if (s)
    {
      size_t i = 0;
      RBLIST_FOR (c, rba->head) assert (c == s[i++]);
      RBLIST_END for (size_t j = 0; j < rba->used; j++)
...
main (void)
{
  ...
  test (rba, "xyfoo064", 8);
  assert (rba->used == 0);
  assert (!rblist_compare
	  (rbacc_to_rblist (rba), rblist_from_string ("xyfoo064")));
